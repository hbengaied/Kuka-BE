    ...
    def run(self): # cette fonction est appelé lorsque kuka a réussi à se connecter
        ...
        while True:
            ...
            if MM.Phrase.Check == True :
                ...
                if MM.Phrase.CheckUp == False :
                    ConversationGoUp.ResponseGoUp(MM.Phrase.MyText[MM.Phrase.Compteur])
                    MM.Phrase.CheckUp = GoUp.VerifGoUp(MM.Phrase.MyText[MM.Phrase.Compteur])
                #Je vais passer caractere suivant
                if MM.Phrase.CheckUp == True and MM.Phrase.Check == True :
...
class ConversationGoUp():
    @staticmethod
    def ResponseGoUp(lettre):
        try:
            tree = ET.parse("input.xml") # je parse le fichier xml
            root = tree.getroot() # je prend la balise root du fichier
